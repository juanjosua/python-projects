...
class Youtube_mp3():
    ...
    def url_search(self, search_string, max_search):
        ...
        result = soup.findAll(attrs={'class': 'yuRUbf'})

        # save soup result into html
        # text_file = open("sample.html", "w", encoding="utf-8")
        # n = text_file.write(r'{}'.format(soup))
        # text_file.close()
...
if __name__ == '__main__':
    ...
    while search != 'q':
        ...
        if search != 'q' and (download == '1' or download == ''):
            ...
            print('\nDownloading {0} (conveniently) from youtube servers.'.format(
                search.title()))
            x.url_search(search, max_search)
            x.get_search_items(max_search)
            song_number = input('Input song number: ')
